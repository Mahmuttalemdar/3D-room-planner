...
Camera::Camera() : m_Position(0.0f,5.0f,-5.0f), m_Target(0.0f,0.0f,0.0f), m_Up(0.0f,1.0f,0.0f), m_LastMouseX(-1), m_LastMouseY(-1), m_Panning(0,0,0), m_Zoom(0,0,0), m_Rotation(0,0,0)
{
	...
	//1. parameter =  kamera oeffnungswinkel = zoom
	//2. parameter = aspect ratio
	//3. parameter = near clipping plane
	//4. parameter = far clipping plane
}
...
void Camera::keyInput( int x, int y, int Button, int State)
{
    if(State == GLUT_DOWN)
    {
        if(m_LastMouseX==-1) m_LastMouseX = x;
